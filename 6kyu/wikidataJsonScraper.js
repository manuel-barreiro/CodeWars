...
const wikidataScraper = async (url) => {
    ...
    const id = Object.keys(response.data.entities)
    const data = response.data.entities[id]

    // Id
    let identificator = id[0]
    console.log(identificator)

    // Label
    let enLabel = data.labels['en']?.value || "No Label";
    console.log(enLabel);

    //  Description
    let enDescr = data.descriptions['en']?.value || "No Description";
    console.log(enDescr);
