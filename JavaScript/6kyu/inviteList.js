...
function getAttendees(peopleInvited, responses){
    ...
    for (let i = 0; i < peopleInvited.length; i++){
      ...
      let invitedPerson = peopleInvited[i]
      let respuestaPersona = responses.find(x => x.name == `${invitedPerson}`)
      let respuesta = (respuestaPersona) ? respuestaPersona.response : 'no answer'
    
      
      if (respuesta === 'declined') {
        peopleInvited.splice(i,1)
      }
