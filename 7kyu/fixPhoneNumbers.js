...
function isItANum(str) { 
  let strToNumArr = str.split('').map(function(x){
    if (x === ' ') {
        return NaN
    }
    else{
  ...
  let onlyNumArr = strToNumArr.filter(x => Number.isNaN(x) === false);
  let phoneNumber = onlyNumArr.join('')
  
  if (phoneNumber.length !== 11 || onlyNumArr[0] !== 0 ){
